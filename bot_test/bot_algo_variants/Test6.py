...
def lidar_thread():
    ...
    try:
        ...
        # Uncomment if explicit motor start is required:
        # lidar.start_motor()
        # time.sleep(3)
...
# ----------------------- Main Control Loop (State Machine with Stuck Timer) -----------------------

def main_control_loop():
    global running, ball_target
    print("Starting main control loop...")
    stuck_timer = 0.0
    last_cmd = None  # Store the last command issued ("left", "right", "forward", "collect", etc.)
    ...
    while running:
        ...
        # Determine current command (as string)
        current_cmd = None
        ...
        if obstacle:
            ...
            if maneuver == "left":
                ...
                current_cmd = "pivot"  # if ambiguous, use pivot
        else:
        ...
        # If the same turning/collection command persists for over 2 seconds, override with forward
        if stuck_timer > 2.0:
            ...
            if current_cmd == "left":
                ...
                kill()
                # In collection mode, decide direction based on ball position.
                with ball_target_lock:
                    cx = current_ball["center"][0]
                if cx > 900:
                    right()
