...
def main_control_loop():
    ...
    stuck_timer = 0
    last_direction = None  # tracks the last command ("left", "right", "forward", or "collect")
    while running:
        ...
        if obstacle:
            ...
            if maneuver == "left":
                ...
                leftPivot()
            # Reset object detection stuck counter if in obstacle mode.
            stuck_timer = 0
            last_direction = None
        else:
            # Priority 2: Object detection and collection.
            with ball_target_lock:
            ...
            if current_ball is not None:
                ...
                if top_val > 600:
                    # Ball is close enough: trigger collection.
                    kill()  # Stop movement before collection.
                    if cx > 900:
                    ...
                    # Adjust course to center the ball.
                    if cx > 870:
                        current_direction = "right"
                        right()
                    elif cx < 430:
                ...
                # If we're repeating the same command, increment the stuck timer.
                if last_direction == current_direction:
                    stuck_timer += 0.1  # approx. time per loop iteration
                else:
                    stuck_timer = 0
                    last_direction = current_direction

                # If stuck in the same turning state for over 2 seconds, override with a forward command.
                if stuck_timer > 2.0:
                    print("Stuck turning detected; overriding with forward drive")
                    forward()
                    stuck_timer = 0
                    last_direction = None
            else:
